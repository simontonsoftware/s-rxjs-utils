...
describe('skipAfter()', () => {
  ...
  it('cleans up subscriptions to `skip$`', () => {
    const skip$ = new Subject();
    const source = new Subject();
    const next = jasmine.createSpy();

    const subscription1 = source.pipe(skipAfter(skip$)).subscribe(next);
    const subscription2 = source.pipe(skipAfter(skip$)).subscribe(next);
